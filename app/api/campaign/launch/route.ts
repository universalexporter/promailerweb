...
export async function POST(req: Request) {
  try {
    // 1. Auth — same Bearer API key pattern as /api/send
    ...
    // 2. Parse the payload the desktop app will send
    ...
    // 3. Resolve the user via their API key (same as /api/send)
    ...
    if (jobError || !job) {
      console.error('launch: job insert failed', jobError)
      return NextResponse.json({ error: 'Could not create send job' }, { status: 500 })
    }
